...
B_info = [
    ...
    ['AP_Capacity_factor',          60, 'ANC/PNC Capacity Factor',              None],
    ...
    ['PNC_effect',                  0.2, 'PNC effect on health',                None],
    ...
    ['Q_Health_L4_L2_difference',   1.0, 'Difference L4-L2 Health Effect',      None],
...
def calc_y(S_values, FP_values, B_values, C_values, P_values):  # values from the sliders
    ...
    num_deliver, num_deliver_avg, neg_HO_avg, num_post_neg_avg, num_post_baby_neg_avg = \
        np.zeros((nt, 4)), np.zeros((nt, 4)), np.zeros((nt, 4)), np.zeros((nt, 1)), np.zeros((nt, 1))
    pos_HO, neg_HO, L2_D_Capacity, L4_D_Capacity, L2_AP_Capacity, num_ANC_avg, num_PNC_avg, num_ANC_visits, \
        num_PNC_visits, num_post_neg, num_post_baby_neg = \
        np.zeros([nt, 4]), np.zeros([nt, 4]), np.ones(nt), np.ones(nt), np.ones(nt), np.zeros([nt, 1]), np.zeros(
            [nt, 1]), np.zeros([nt, 1]), np.zeros([nt, 1]), np.zeros([nt,1]), np.zeros([nt,1])
    ...
    for t in range(0, nt - 1):
        ...
        gest_age, health, anc_visit, pnc_visit, delivery, prob_delivery, facility = [], [], [], [], [], [], []

        if t > parameters['t_change']:  # IF TIME IS LARGER THAN THE t_change SLIDER VALUE
            for idx, name in FP_idx_names:
                globals()[name] = FP_change[
                    idx]  # then use the SLIDER value for the F-parameter, e.g., Visibility = 0.0
            for idx, name in B_idx_names:
        ...
        L2_AP_Capacity[t] = (1 - L2_DC[t]) * AP_Capacity_factor
        ...
        dL4_HR_in = L4_HF[t]
        S_FR_target = S_FR_target_0 * P_RR[t] * logistic(
            [Employee_incentives, Strong_referrals, Training_incentives, 3])
        ...
        L2_deliveries, ANC_visits, PNC_visits, Change_neg, Change_baby_neg = 0, 0, 0, 0, 0
        for mother in mothers:
            ...
            mother.increase_age(l4_quality_avg, l2_quality_avg, neg_health_outcomes, L2_net_capacity, AP_net_capacity,
                                mothers, t)
            if mother.delivered:
            ...
            if mother.PNC_visited:
                PNC_visits += 1
                pnc_visit.append(1)
                mother.PNC_visited = False
            if mother.post_delivery == 0:
                Change_neg += 1
                mother.post_delivery = None
            if mother.post_delivery_baby == 0:
                Change_baby_neg += 1
                mother.post_delivery_baby = None
        ...
        num_post_baby_neg[t + 1] = Change_baby_neg * B['Population_factor'] / B['Outcomes_factor']
        ...
        num_post_baby_neg_avg[t + 1, :] = window_average(num_post_baby_neg, t + 2, time_window, linear_weights=True)
        ...
        if np.prod(deliveries_t[:2]) == 0:
            ...
            L2_4_health_outcomes = 1 - sum(neg_HO_t[1:3]) / sum(
                deliveries_t[1:3])  # don't include neg_HO_t[3] = total neg HO
    ...
    return t_all, y_t, \
        ...
        [pos_HO, neg_HO_avg], probs, [num_near_miss_avg, DALY, num_post_neg_avg, num_post_baby_neg_avg]  # pos_HO not used
...
B_sliders = many_sliders(B_label, 'B_slider', B_0, B_initial, np.zeros(len(B_0)), np.array(B_0) * 4, num_rows=6,
                         num_cols=4, width=3)
# many_sliders(labels, type used in Input() as an identifier of group of sliders, initial values, min, max, ...
C_sliders = many_sliders(C_label, 'C_slider', C_0, C_initial, np.zeros(len(C_0)), np.ones(len(C_0)), num_rows=3,
                         num_cols=4, width=3)
...
app.layout = html.Div(style={'backgroundColor': '#f6fbfc'}, children=[
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Col([html.H5('Positive Combination Factors', style={'fontSize': fsize, 'color': fcolor}),
                     FP_combination_sliders], width=12),
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Div([
                make_slider(0, 'Time when factor changes will take place', 'P_slider', parameters['t_change'], None, 0,
                            nt)
            ]),
        ], width=3),
        dbc.Col([
...
@app.callback(
    ...
    [Input({'type': 'S_slider', 'index': ALL}, 'value'),
     # get all S-slider values, pass as 1st argument to update_graph()
...
def update_graph(S_values, FP_values, B_values, C_values, P_values, FP_max):  # each argument is one of Input(...)
    ...
    # DELIVERIES
    labels = ['Level 4/5', 'Level 2/3', 'Home', 'Total', 'Capacity 4/5', 'Capacity 2/3', 'AP capacity', 'ANC visits',
              'PNC visits']  # total is unused
    line_color = [0, 1, 2, 5, 0, 1, 5, 5, 1]
    line_dash = ['none', 'none', 'none', 'dash', 'dash', 'dash', 'dash', 'none', 'none']
    k_plot = [0, 1, 2, 4, 5, 3, 6, 7, 8]
    fig_2B = {
        'data': [{
            'x': t_all[2:],
            'y': num_d[k][2:],
    ...
    # SMO
    NM_DALY = np.concatenate((SMO[0][:, 0:3], SMO[1][:, 0:3], SMO[2], SMO[3]), axis=1)
    labels = ['Home near miss', 'L2 near miss', 'L4 near miss', 'Home DALY', 'L2 DALY', 'L4 DALY',
              'Postbirth Maternal Neg HO', 'Postbirth Baby Neg HO']
    line_color = [2, 1, 0, 2, 1, 0, 3, 4]
    line_dash = ['dash', 'dash', 'dash', 'none', 'none', 'none', 'none', 'none']
    k_plot = [2, 1, 0, 5, 4, 3, 6, 7]
